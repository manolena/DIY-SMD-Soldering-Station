...
#define DELAY_MAIN_LOOP 	500//150
...
#define TURN_OFF_TIME 1800000    //30min  1800000
...
//double aggKp=0.5, aggKi=0.9, aggKd=0.2;
//double consKp=0.5, consKi=0.9, consKd=0.2;


double aggKp=450, aggKi=260, aggKd=200;
double consKp=450, consKi=260, consKd=200;
...
const int numReadings = 150;//30
int readings[numReadings];      // the readings from the analog input
int readIndex = 0;              // the index of the current reading
int total = 0;                  // the running total
float adcValue;

long update_last, on_time = 0;
char heater_mode = OFF;
...
{ 
  ...
  Serial.println("C:\\Users\\Administrator\\Documents\\DXP\\AVR SOLDERING IRON\\FIRMWARE\\IRON_15");
  ...
  horizontalBar();
#endif 
  Input = getTemperature()-ADC_TO_TEMP_OFFSET;
  Setpoint = ADC_TO_TEMP_OFFSET;
...
{  
  ...
  if(actual_temperature > MAX_TEMP+100)
  {    
    pwm = 0;
    digitalWrite(HEAT_LED, LOW);
...
int getTemperature()
{  
  digitalWrite(PWMpin, 0);		//switch off heater
